...
import { AnyExtension, Editor, Extension } from "@tiptap/react";
...
import { InsertMediaNiceDialog } from "../../dialogs/insert-media-dialog";
...
export const LessonContentEditor = (props: LessonContentEditorProps) => {
  ...
  return (
    <ContentEditor
      ...
      {...editorProps}
      toolbar={
        editorProps.toolbar !== undefined ? editorProps.toolbar : EditorToolbar
      }
    />
  );
};
...
const CustomMediaInsertDropdown = () => {
  ...
  const handleInsertMedia = (
    fileType: "document" | "image" | "video" | "audio" | "json" | "all"
  ) => {
    if (editor) {
