        ...
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              ...
              {t("dashboard:ai.resource_status")}
            </DialogTitle>
          </DialogHeader>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="font-semibold">
                          {status.exists ? t("dashboard:ai.synced") : t("dashboard:ai.not_synced")}
                        </div>
                        <div className="text-xs text-muted-foreground">
                          {status.exists
                            ? t("dashboard:ai.resource_available_in_ai")
                            : t("dashboard:ai.resource_not_added_yet")}
                        </div>
                      </div>
                    </div>
                  ...
                  <CardContent className="pt-4 pb-4">
                    <div className="grid grid-cols-2 gap-4 text-sm">
                      {status.metadata.totalChunks !== undefined && (
                        <div>
                          <div className="text-muted-foreground">{t("dashboard:ai.chunks")}</div>
                          <div className="font-semibold">{status.metadata.totalChunks}</div>
                        </div>
                      )}
                      {status.metadata.vectorCount !== undefined && (
                        <div>
                          <div className="text-muted-foreground">{t("dashboard:ai.vectors")}</div>
                    ...
                    {status.syncedAt && (
                      <div className="mt-3 pt-3 border-t text-xs text-muted-foreground">
                        {t("dashboard:ai.last_synced")}: {new Date(status.syncedAt).toLocaleString()}
                      </div>
                    )}
                  </CardContent>
                </Card>
                ...
                >
                  {adding ? (
                    ...
                    <>
                      ...
                      {t("dashboard:ai.add_to_resource")}
                    </>
                  )}
                </Button>
              )}
