...
>(({ courseId }) => {
  ...
  const handleDelete = useCallback((resource: RagResource) => {
      ...
      <DialogContent className="max-w-3xl">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            {t("dashboard:ai.rag_resources")}
          </DialogTitle>
          <DialogDescription>
            {t("dashboard:ai.rag_resources_description")}
          </DialogDescription>
        </DialogHeader>

        <div className="flex justify-between items-center mb-4">
          <div className="text-sm text-muted-foreground">
            {resources.length} {t("dashboard:ai.resources_found")}
          </div>
          <div className="flex gap-2">
            ...
            >
              ...
              {isSyncing ? t("dashboard:ai.syncing") : t("dashboard:ai.sync")}
            </Button>
            <Button
              type="button"
              variant="outline"
        ...
        <ScrollArea className="h-[400px] pr-4">
          {isLoading && resources.length === 0 ? (
            ...
            <div className="flex items-center justify-center h-32 text-muted-foreground">
              {t("dashboard:ai.no_resources_found")}
            </div>
          ) : (
                      ...
                      <div className="flex flex-wrap gap-2 mb-2">
                        ...
                        {resource.sectionsCount !== undefined && (
                          <Badge variant="outline">
                            {resource.sectionsCount} {t("dashboard:ai.sections")}
                          </Badge>
                        )}
                      </div>
                      {resource.checksum && (
