...
export default function Page() {
  ...
  const breadcrumbs = useMemo(() => {
    const breadcrumbs = [
      ...
      {
        label: initialCourse
          ? truncate(initialCourse.title, 20) || "..."
          : "...",
        href: "#",
      },
    ];
  ...
  const enrollmentRequestStatsQuery =
    trpc.lmsModule.enrollmentRequest.stats.useQuery(
      {
    ...
    <DashboardContent breadcrumbs={breadcrumbs}>
          ...
          <div className="flex flex-wrap gap-2 items-center">
            <Select value={timeRange} onValueChange={setTimeRange}>
              <SelectTrigger className="w-[140px]" size="sm">
                <SelectValue
                  placeholder={t("course:detail.select_time_range")}
                />
              </SelectTrigger>
              <SelectContent>
            ...
            <Button variant="outline" size="sm" asChild>
              <Link
                href={`/dashboard/lms/courses/${initialCourse._id}/content`}
              >
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end">
                <DropdownMenuItem asChild>
                  <Link
                    href={`/dashboard/lms/courses/${initialCourse._id}/manage`}
                  >
                ...
                <DropdownMenuItem asChild>
                  <Link
                    href={`/dashboard/lms/courses/${initialCourse._id}/cohorts`}
                  >
      ...
      {enrollmentRequestStatsQuery.isLoading ? (
        ...
        <Card
          ...
          onClick={() => {
            router.push(
              `/dashboard/lms/courses/${initialCourse._id}/requests?filter[status]=${ApprovalStatusEnum.PENDING}`
            );
          }}
        >
          <CardHeader>
            <CardTitle className="flex items-center justify-between text-base">
              ...
              <Badge variant="default">
                {enrollmentRequestStatsQuery.data?.pending}
              </Badge>
            </CardTitle>
          </CardHeader>
          <CardContent>
            <p className="text-sm text-muted-foreground">
              {t("course:requests.pending_desc", {
                count: enrollmentRequestStatsQuery.data?.pending,
              })}
            </p>
          </CardContent>
        </Card>
      ...
      <Card
        className="mt-6 cursor-pointer hover:bg-accent/50 transition-colors"
        onClick={() => {
          router.push(`/dashboard/lms/courses/${initialCourse._id}/students`);
        }}
      >
        <CardHeader>
          <CardTitle className="flex items-center justify-between text-base">
        ...
        <CardContent>
          <p className="text-sm text-muted-foreground">
            {t("course:detail.view_enrolled_students")}
          </p>
        </CardContent>
      </Card>
