...
export default function CourseCohorts() {
  ...
  const handleDelete = useCallback((cohort: ItemType) => {
    NiceModal.show(DeleteConfirmNiceDialog, {
      title: t("course:cohorts.delete_dialog.title"), 
      message: t("course:cohorts.delete_dialog.message", { name: cohort.title }),
    })
      .then((result) => {
    ...
    return [
      ...
      },
      {
        ...
        cell: ({ row }) => {
          ...
          return <div>{instructor?.fullName || t("course:cohorts.table.no_instructor")}</div>;
        },
      },
      {
        accessorKey: "status",
          ...
          if (!maxCap) {
            return (
              <div className="flex items-center gap-2">
                <span className="text-sm">{currentCount} / {t("common:unlimited")}</span>
              </div>
            );
          }
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="ghost" size="icon">
                  ...
                  <span className="sr-only">{t("course:cohorts.actions.open_menu")}</span>
                </Button>
              </DropdownMenuTrigger>
              <DropdownMenuContent align="end">
                <DropdownMenuItem asChild>
                  <Link href={`/dashboard/lms/cohorts/${cohort._id}`} target="_blank">
                    ...
                    {t("course:cohorts.actions.view_details")}
                  </Link>
                </DropdownMenuItem>
                <DropdownMenuItem asChild>
                  <Link href={`/dashboard/lms/cohorts/${cohort._id}`} target="_blank">
    ...
    return (
      <DashboardContent
        breadcrumbs={[
          ...
          { label: "...", href: "#" },
          { label: t("course:cohorts.breadcrumb_cohorts"), href: "#" },
        ]}
      >
    ...
    return (
      ...
      >
        <div className="text-center py-8">
          <p className="text-muted-foreground">{t("course:cohorts.not_found")}</p>
        </div>
      </DashboardContent>
    );
  }

  const isTableLoading = loadListQuery.isFetching;
  ...
  return (
    ...
    >
      <HeaderTopbar
        backLink={true}
        header={{
          title: t("course:cohorts.title"),
          subtitle: t("course:cohorts.subtitle", { courseName: initialCourse.title || "" }),
        }}
        rightAction={
          <CreateButton onClick={handleCreateCohort} text={t("course:cohorts.add_cohort")} />
        }
      />
      <Card>
        <CardContent>
          <div className="flex flex-col gap-4 pt-6">
            <div className="flex flex-wrap gap-4">
              <Input
                placeholder={t("course:cohorts.search_placeholder")}
                value={searchQuery}
              ...
              <Select value={statusFilter} onValueChange={(value) => setStatusFilter(value as CohortStatusEnum | "all")}>
                <SelectTrigger className="w-[180px]">
                  <SelectValue placeholder={t("course:cohorts.filter_status")} />
                </SelectTrigger>
                <SelectContent>
            ...
            <DataTable table={table} isLoading={isTableLoading}>
