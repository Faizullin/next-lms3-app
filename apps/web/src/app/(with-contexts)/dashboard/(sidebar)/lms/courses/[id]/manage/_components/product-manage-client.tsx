...
const createProductSchema = (t: (key: string, params?: any) => string) => z.object({
  title: z.string()
    .min(1, t("error:validation.required", { field: t("common:title") }))
    .max(255, t("error:validation.max_length", { field: t("common:title"), max: 255 })),
...
export default function ProductManageClient() {
  ...
  const { t } = useTranslation(["course", "common", "error"]);
  ...
  const breadcrumbs = [
    { label: t("dashboard:lms.course.module_title"), href: "/dashboard/lms/courses" },
    {
        ...
        <MediaSelector
          ...
          functions={{
            uploadFile: async (files: File[], _type: string, _storageProvider?: string, _caption?: string) => {
              ...
              toast({
                title: t("common:success"),
                description: t("common:toast.item_uploaded_successfully", { item: t("course:manage.featured_image") }),
              });
            ...
            removeFile: async (mediaId: string) => {
              ...
              toast({
                title: t("common:success"),
                description: t("common:toast.item_removed_successfully", { item: t("course:manage.featured_image") }),
              });
            }
          }}
        />
      ...
      <div className="space-y-6" id="publish">
        <div className="flex items-center justify-between">
          <div className="space-y-0.5">
            <Label htmlFor="course-published" className="text-base font-semibold">{t("common:published")}</Label>
            <p className="text-sm text-muted-foreground">
            ...
            NiceModal.show(DeleteConfirmNiceDialog, {
              ...
              .then((result) => {
                if (result.reason === "confirm") {
                  return deleteCourseMutation.mutateAsync({ id: product._id });
                }
              })
              .catch((error) => {
