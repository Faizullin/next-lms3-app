...
export default function QuizQuestions() {
    ...
    trpc.lmsModule.quizModule.quizQuestions.delete.useMutation({
      onSuccess: async () => {
        toast({
          ...
          description: t("dashboard:lms.quizzes.toast.question_deleted"),
        });
  ...
  const handleDeleteQuestion = useCallback(
    async (question: QuestionType) => {
      if (!confirm(t("dashboard:lms.quizzes.delete_question_confirm"))) return;
  ...
  const getQuestionTypeLabel = (type: QuestionTypeEnum) => {
    switch (type) {
      case QuestionTypeEnum.MULTIPLE_CHOICE:
        return t("dashboard:lms.quizzes.questions.types.multiple_choice");
      case QuestionTypeEnum.SHORT_ANSWER:
        return t("dashboard:lms.quizzes.questions.types.short_answer");
      default:
        return type;
    }
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        <div className="flex items-center gap-2">
          ...
          <span className="font-medium">{questions.length} {t("dashboard:lms.quizzes.questions.title")}</span>
        </div>
        <Badge variant="outline">{t("dashboard:lms.quizzes.settings.total_points")}: {totalPoints}</Badge>
      </div>
              ...
              <div className="p-4 border-b bg-muted/30">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-medium">{t("dashboard:lms.quizzes.questions.title")}</span>
                  </div>
              ...
              <ScrollArea className="h-[calc(600px-57px)]">
                {questions.length === 0 ? (
                  <div className="p-8 text-center text-muted-foreground">
                    ...
                    <p>{t("dashboard:lms.quizzes.questions.no_questions")}</p>
                    <p className="text-sm">{t("dashboard:lms.quizzes.questions.no_questions_desc")}</p>
                  </div>
                ) : (
                  <div className="p-2">
            ...
            <div className="lg:col-span-2 bg-muted/10">
              {isEditing ? (
                ...
                <div className="p-8 text-center h-full flex flex-col items-center justify-center">
                  ...
                  <h3 className="text-lg font-medium mb-2">{t("dashboard:lms.quizzes.questions.no_questions")}</h3>
                  <p className="text-muted-foreground mb-4">
                    {t("dashboard:lms.quizzes.questions.no_questions_desc")}
                  </p>
                  <Button onClick={handleCreateQuestion}>
                    <Plus className="h-4 w-4 mr-2" />
                    {t("dashboard:lms.quizzes.questions.add_question")}
                  </Button>
                </div>
              )}
            </div>
  ...
  const createQuestionMutation =
    trpc.lmsModule.quizModule.quizQuestions.create.useMutation({
      onSuccess: async () => {
        toast({
          title: t("common:success"),
          description: t("dashboard:lms.quizzes.toast.question_created"),
        });
        await trpcUtils.lmsModule.quizModule.quizQuestions.list.invalidate();
        onSuccess();
      },
  ...
  const updateQuestionMutation =
    trpc.lmsModule.quizModule.quizQuestions.update.useMutation({
      onSuccess: async () => {
        toast({
          title: t("common:success"),
          description: t("dashboard:lms.quizzes.toast.question_updated"),
        });
        await trpcUtils.lmsModule.quizModule.quizQuestions.list.invalidate();
        onSuccess();
      },
    ...
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        <h3 className="text-lg font-semibold">
          {isEdit ? t("common:edit") : t("common:create")} {t("dashboard:lms.quizzes.questions.title")}
        </h3>
      ...
      <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-6">
        <FieldGroup>
            ...
            <Controller
              ...
              render={({ field, fieldState }) => (
                <Field data-invalid={fieldState.invalid}>
                  <FieldLabel htmlFor="question-type">{t("dashboard:lms.quizzes.questions.question_type")} *</FieldLabel>
                  <Select
                    value={field.value}
          ...
          <Controller
            ...
            render={({ field, fieldState }) => (
              <Field data-invalid={fieldState.invalid}>
                <FieldLabel htmlFor="question-text">{t("dashboard:lms.quizzes.questions.question_text")} *</FieldLabel>
                <Textarea
                  {...field}
                  id="question-text"
                  placeholder={t("dashboard:lms.quizzes.questions.question_text_placeholder")}
                  rows={3}
                  aria-invalid={fieldState.invalid}
                />
                {fieldState.invalid && <FieldError errors={[fieldState.error]} />}
          ...
          <Controller
            ...
            render={({ field, fieldState }) => (
              <Field data-invalid={fieldState.invalid}>
                <FieldLabel htmlFor="question-explanation">{t("dashboard:lms.quizzes.questions.explanation")}</FieldLabel>
                <Textarea
                  {...field}
                  id="question-explanation"
                  placeholder={t("dashboard:lms.quizzes.questions.explanation_placeholder")}
                  rows={2}
                  aria-invalid={fieldState.invalid}
                />
                {fieldState.invalid && <FieldError errors={[fieldState.error]} />}
...
function MultipleChoiceFields({
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <FieldLabel className="text-base">{t("dashboard:lms.quizzes.questions.options")} *</FieldLabel>
        <Button type="button" variant="outline" size="sm" onClick={addOption}>
          <Plus className="h-3 w-3 mr-1" />
          {t("dashboard:lms.quizzes.questions.add_option")}
        </Button>
      </div>
                ...
                render={({ field, fieldState }) => (
                  <Field data-invalid={fieldState.invalid}>
                    <Input
                      ...
                      placeholder={t("dashboard:lms.quizzes.questions.option_placeholder", { number: index + 1 })}
                      aria-invalid={fieldState.invalid}
                    />
                    {fieldState.invalid && <FieldError errors={[fieldState.error]} />}
                  </Field>
                ...
                render={({ field }) => (
                  <div className="flex items-center gap-2">
                    ...
                    <FieldLabel htmlFor={`option-${index}-correct`} className="text-sm font-normal cursor-pointer">
                      {t("dashboard:lms.quizzes.questions.mark_correct")}
                    </FieldLabel>
                  </div>
                )}
              />
...
function ShortAnswerFields({
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <FieldLabel className="text-base">{t("dashboard:lms.quizzes.questions.correct_answers")} *</FieldLabel>
        <Button
          type="button"
          variant="outline"
          size="sm"
        ...
        >
          <Plus className="h-4 w-4 mr-2" />
          {t("dashboard:lms.quizzes.questions.add_answer")}
        </Button>
      </div>
      <div className="space-y-3">
              ...
              render={({ field, fieldState }) => (
                <Field className="flex-1" data-invalid={fieldState.invalid}>
                  <Input
                    ...
                    placeholder={t("dashboard:lms.quizzes.questions.answer_placeholder", { number: index + 1 })}
                    value={field.value || ""}
