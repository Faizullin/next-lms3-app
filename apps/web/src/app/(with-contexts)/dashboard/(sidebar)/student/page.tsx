...
import { BookOpen, Calendar, CheckCircle, ClipboardList, Clock, TrendingUp, Eye, Video } from "lucide-react";
                                ...
                                {isLoading ? (
                                    ...
                                    <div className="space-y-3">
                                        {upcomingEvents.slice(0, 5).map((event) => (
                                            <StudentUpcomingEventCard key={event._id} event={event} />
                                        ))}
...
type StudentUpcomingEvent = GeneralRouterOutputs["lmsModule"]["student"]["getDashboardStats"]["upcomingEvents"][number];

const StudentUpcomingEventCard = ({ event }: { event: StudentUpcomingEvent }) => {
    const { t } = useTranslation(["dashboard", "common"]);
    const meetingUrl = event.location?.meetingUrl;
    const isLiveSession = event.type === ScheduleTypeEnum.LIVE_SESSION;
