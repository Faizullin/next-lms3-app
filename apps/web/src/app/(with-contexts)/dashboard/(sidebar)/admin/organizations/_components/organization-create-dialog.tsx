...
const createOrganizationSchema = (t: (key: string, params?: any) => string) => z.object({
  ...
  email: z.string()
    .min(1, t("error:validation.required", { field: t("common:email") }))
    .email(t("error:validation.invalid_email")),
  description: z.string()
    .max(1000, t("error:validation.max_length", { field: t("common:description"), max: 1000 }))
    .optional(),
  phone: z.string()
    .max(50, t("error:validation.max_length", { field: t("common:phone"), max: 50 }))
    .optional(),
  address: z.string()
    .max(500, t("error:validation.max_length", { field: t("common:address"), max: 500 }))
    .optional(),
});
    ...
    trpc.siteModule.organization.create.useMutation({
      onSuccess: () => {
        toast({
          ...
          description: t("common:toast.item_created_successfully", { item: t("admin:organizations.create_organization") }),
        });
        control.hide();
        form.reset();
    ...
    >
      <FieldGroup>
        ...
        <Controller
          ...
          render={({ field, fieldState }) => (
            <Field data-invalid={fieldState.invalid}>
              <FieldLabel>{t("common:description")} ({t("common:optional")})</FieldLabel>
              <Textarea
                {...field}
        ...
        <div className="grid grid-cols-2 gap-4">
          <Controller
            ...
            render={({ field, fieldState }) => (
              <Field data-invalid={fieldState.invalid}>
                <FieldLabel>{t("common:phone")} ({t("common:optional")})</FieldLabel>
                <Input
                  {...field}
                  type="tel"
          ...
          <Controller
            ...
            render={({ field, fieldState }) => (
              <Field data-invalid={fieldState.invalid}>
                <FieldLabel>{t("common:address")} ({t("common:optional")})</FieldLabel>
                <Input
                  {...field}
