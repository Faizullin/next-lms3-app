  ...
  const onSubmit = async (data: GeneralSettingsFormData) => {
    try {
      ...
      toast({
        ...
        description: t("common:toast.item_updated_successfully", { item: t("common:settings") }),
      });
    } catch (error) {
      toast({
  ...
  return (
    <div className="space-y-8">
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
        <FieldGroup>
          ...
          <Controller
            ...
            render={({ field, fieldState }) => (
              <Field data-invalid={fieldState.invalid}>
                <FieldLabel>{t("common:subtitle")}</FieldLabel>
      ...
      <div className="space-y-4">
        <h3 className="text-lg font-semibold">{t("common:logo")}</h3>
        <MediaSelector
            media={logo}
