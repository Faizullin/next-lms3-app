...
export default function CourseLayoutContent({
  ...
  const { t } = useTranslation(["course", "frontend"]);
  ...
  const selfEnrollMutation = trpc.lmsModule.enrollment.selfEnrollCourse.useMutation({
    onSuccess: () => {
      toast({
        title: t("course:enrollment.success"),
        description: t("course:enrollment.success_desc"),
      });
    ...
    onError: (error) => {
      toast({
        title: t("course:errors.enrollment_failed"),
        description: error.message || t("course:errors.generic"),
        variant: "destructive",
      });
    },
  });
  ...
  const requestEnrollMutation = trpc.lmsModule.enrollment.requestEnrollment.useMutation({
    onSuccess: () => {
      toast({
        title: t("course:enrollment.request_success"),
        description: t("course:enrollment.request_success_desc"),
      });
      trpcUtils.lmsModule.enrollment.getMembership.invalidate({ courseId: initialCourse._id });
    },
    onError: (error) => {
      toast({
        title: t("course:errors.request_failed"),
        description: error.message || t("course:errors.generic"),
        variant: "destructive",
      });
    },
  ...
  const progressPercentage = loadMembershipQuery.data?.progress?.progressPercentage || 0;
  const cohort = loadMembershipQuery.data?.enrollment?.cohort;

  const handleSelfEnroll = useCallback(() => {
    if (!profile?.id) {
      toast({
        title: t("course:enrollment.login_required"),
        description: t("course:enrollment.login_required_desc"),
      });
      setTimeout(() => {
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-5">
              ...
              {isEnrolled ? (
                ...
                >
                  <Button variant="default" size="default" className="w-full">
                    ...
                    {t("frontend:course_detail.continue_learning")}
                  </Button>
                </Link>
              ) : (
                <>
                      ...
                      courseDetailedData?.allowSelfEnrollment ? (
                        ...
                        >
                          ...
                          {selfEnrollMutation.isPending 
                            ? t("course:public.enrolling") 
                            : t("course:public.start_learning")}
                        </Button>
                      ) : (
                        <Button
                          variant="outline"
                        ...
                        >
                          ...
                          {requestEnrollMutation.isPending 
                            ? t("course:public.requesting") 
                            : t("course:public.request_enrollment")}
                        </Button>
                      )
                    )
                  ) : defaultPaymentPlan ? (
                    <Button variant="default" size="default" className="w-full" disabled>
                      {t("course:public.buy_for")} {getPriceDisplay()}
                    </Button>
                  ) : (
            ...
            {!isEnrolled && (
              <div className="space-y-3 mt-5 pt-5 border-t border-gray-100 dark:border-gray-700">
                <div className="font-medium text-gray-900 dark:text-gray-100">{t("course:public.course_includes")}</div>
                <div className="flex items-center text-gray-700 dark:text-gray-300">
                  <BookOpen className="h-4 w-4 stroke-1.5" /> 
                  <span className="ml-2">
                    {t("course:public.lessons_number", { count: courseDetailedData.statsLessonCount })}
                  </span>
                </div>
              </div>
            )}
