...
import { getStorageProvider, type StorageProviderType } from "@/server/services/storage-provider";
...
const VALID_STORAGE_PROVIDERS: StorageProviderType[] = ["local", "cloudinary", "vercel"];
...
export async function POST(request: NextRequest) {
  try {
    ...
    const storageProviderParam = formData.get("storageProvider") as string | null;
    ...
    let storageProvider: StorageProviderType | undefined;
    if (storageProviderParam) {
      if (VALID_STORAGE_PROVIDERS.includes(storageProviderParam as StorageProviderType)) {
        storageProvider = storageProviderParam as StorageProviderType;
      } else {
        throw new ValidationException(`Invalid storage provider: ${storageProviderParam}`);
      }
    }
    ...
    const attachment = await getStorageProvider(storageProvider).uploadFile(
      {
        file,
