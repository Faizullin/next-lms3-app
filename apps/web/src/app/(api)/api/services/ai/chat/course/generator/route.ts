...
import { createTiptapContent, buildConversionInstructions } from "@/lib/ai/prompts/tiptap-helpers";
  ...
  try {
    ...
    if (data.useWebSearch) {
      ...
      try {
        ...
        console.log("[researchResult]:", researchResult.text);
...
async function handleApproveStructure(
  ...
  const course = await CourseModel.create({
    ...
    description: createTiptapContent({ type: "doc", content: [{ type: "paragraph", content: [{ type: "text", text: structure.description }] }] }),
...
Return ONLY valid TipTap JSON format.`;
        ...
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
          try {
            ...
            tiptapJson = JSON.parse(jsonText);
            console.log(`[LESSON] ✓ Generated ${lessonData.title} on attempt ${attempt}`);
            break;
          } catch (error) {
            console.error(`[LESSON] Attempt ${attempt}/${maxRetries} failed for ${lessonData.title}:`, error);
        ...
        const tiptapContent = createTiptapContent(tiptapJson);
