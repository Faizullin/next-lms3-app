...
function expandRecurringEvents(
  ...
  const expandedEvents: IExpandedScheduleEvent[] = [];
  ...
  for (const event of events) {
    if (
      ...
      if (
        new Date(event.startDate) >= startRange &&
        new Date(event.startDate) <= endRange
      ) {
        expandedEvents.push({
          event: event,
    ...
    while (currentDate <= endRange && currentDate <= recurrenceEnd) {
      ...
      if (event.recurrence.type === RecurrenceTypeEnum.DAILY) {
        ...
        currentDate.setDate(
          currentDate.getDate() + 7 * event.recurrence.interval
        );
      } else if (event.recurrence.type === RecurrenceTypeEnum.MONTHLY) {
...
export const instructorRouter = router({
  ...
  getDashboardStats: protectedProcedure
    ...
    .query(async ({ ctx }) => {
      ...
      const courseQuery = buildInstructorCourseQuery(
        orgId,
        userId,
        canSeeAllCourses
      );
      ...
      const [courseStats, recentCourses, scheduleEvents, instructorCourseIds] =
        await Promise.all([
          ...
          CourseModel.find(courseQuery)
            .select(
              "_id title shortDescription published publishedAt chapters statsEnrollmentCount statsAverageRating"
            )
            .sort({ updatedAt: -1 })
            .limit(5)
            .lean(),
          ...
          ScheduleEventModel.find({
            ...
            .select(
              "_id title startDate endDate type status recurrence allDay entity instructorId cohortId"
            )
            .sort({ startDate: 1 })
            .lean(),
      ...
      const pendingSubmissionsResult =
        await AssignmentSubmissionModel.aggregate([
          {
            $match: {
              orgId,
      ...
      const upcomingEvents = expandRecurringEvents(
        scheduleEvents,
        now,
        endOfWeek
      ).slice(0, 10);
