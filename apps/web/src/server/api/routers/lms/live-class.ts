...
import { IScheduleEventHydratedDocument, ScheduleEventModel } from "@workspace/common-logic/models/lms/schedule.model";
...
export const liveClassRouter = router({
  ...
  getById: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      const liveClass = await LiveClassModel.findOne({
        ...
        .populate<{
          event: Pick<IScheduleEventHydratedDocument, "_id" | "title" | "startDate" | "endDate" | "type">;
        }>("event", "title startDate endDate type")
        .lean();
  ...
  getByEventId: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      const liveClass = await LiveClassModel.findOne({
        "entity.entityType": "schedule",
        "entity.entityId": new mongoose.Types.ObjectId(input.eventId),
        orgId: ctx.domainData.domainObj.orgId,
      })
        .populate<{
          instructor: Pick<
  ...
  create: protectedProcedure
    ...
    .input(
      getFormDataSchema({
        ...
        cohortId: documentIdValidator().optional(),
        eventId: documentIdValidator().optional(),
    ...
    .mutation(async ({ ctx, input }) => {
      let meetingUrl = input.data.meetingUrl;
      
      if (input.data.eventId) {
        const event = await ScheduleEventModel.findOne({
          _id: input.data.eventId,
          orgId: ctx.domainData.domainObj.orgId,
        }).lean();
        
        if (!event) {
          throw new NotFoundException("Schedule Event", input.data.eventId);
        }
        
        if (!meetingUrl && event.location?.meetingUrl) {
    ...
    .query(async ({ ctx, input }) => {
      ...
      const [enrollments, participants] = await Promise.all([
        EnrollmentModel.find({
          ...
          .lean(),
        LiveClassParticipantModel.find({
          liveClassId: input.id,
          orgId: ctx.domainData.domainObj.orgId,
        }).lean(),
      ]);
      ...
      const items = enrollments.map(enrollment => ({
        ...
        participant: participantsMap.get(String(enrollment.userId)) || null,
      }));
  ...
  startSession: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (liveClass.status === LiveClassStatusEnum.LIVE) {
        throw new ConflictException("Live class is already live");
      }
