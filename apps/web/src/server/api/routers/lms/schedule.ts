      ...
      if (input.filter?.cohortId) {
        ...
        if (!cohort) {
          throw new NotFoundException(
            "Cohort",
            input.filter.cohortId as string
          );
        }
        if (
          !checkPermission(ctx.user.permissions, [
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const schedule = await ScheduleEventModel.create({
        ...
        recurrence: {
          ...
          endDate: input.data.recurrence.endDate
            ? new Date(input.data.recurrence.endDate)
            : undefined,
        },
  ...
  update: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (input.data.description !== undefined)
        schedule.description = input.data.description;
      if (input.data.type) schedule.type = input.data.type;
      if (input.data.startDate)
        schedule.startDate = new Date(input.data.startDate);
      ...
      if (input.data.instructorId !== undefined) {
        ...
        schedule.instructorId = new mongoose.Types.ObjectId(
          input.data.instructorId
        );
      }
