  ...
  NiceModal.show(DeleteConfirmNiceDialog, {
    ...
    .then((result) => {
      if (result.reason === "confirm") {
        return deleteMutation.mutateAsync({ id: item._id });
      }
    })
    .catch((error) => {
...
**DO**: Use specialized combobox wrappers from `@/components/form`

```typescript
import { CourseCombobox, UserCombobox } from "@/components/form";

// Course selector with built-in create/edit buttons
<CourseCombobox
  value={course}
  onChange={setCourse}
  showCreateButton={true}  // Default: true
  showEditButton={true}    // Default: true
/>

// User selector with avatar display
<UserCombobox
  value={user}
  onChange={setUser}
  showEditButton={true}    // Default: true
...
**Features**:
- `CourseCombobox`: Auto-fetches courses, default create/edit URLs
- `UserCombobox`: Shows avatar + full name, smart initials generation
- Type-safe discriminated unions for single/multiple selection
- Optional custom handlers via `onCreateClick`/`onEditClick` props
- Located in `apps/web/src/components/form/`
