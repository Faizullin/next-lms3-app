...
export const MediaViewExtension = Node.create<MediaViewOptions>({
  ...
  markdownTokenName: 'mediaView',

  parseMarkdown(token, helpers) {
    const attrs = token.attrs || {};
    const assetId = attrs.assetId || null;
    let minimalAsset = null;
    let minimalDisplay = null;

    try {
      minimalAsset = attrs.asset ? JSON.parse(attrs.asset) : null;
    } catch (e) {
      console.warn('Failed to parse media asset:', e);
    }

    try {
      minimalDisplay = attrs.display ? JSON.parse(attrs.display) : null;
    } catch (e) {
      console.warn('Failed to parse media display:', e);
    }

    const asset = minimalAsset ? {
      url: minimalAsset.url || '',
      caption: minimalAsset.caption || '',
      media: null,
    } : null;

    const display = minimalDisplay ? {
      width: minimalDisplay.width || '100%',
      height: null,
      align: minimalDisplay.align || 'center',
      aspectRatio: null,
    } : {
      width: '100%',
      height: null,
  ...
  renderMarkdown(node, helpers) {
    ...
    const minimalDisplay = display ? {
      width: display.width,
      align: display.align
    } : {};
    
    const assetJson = JSON.stringify(minimalAsset);
    const displayJson = JSON.stringify(minimalDisplay);
    
    return `:::media {assetId="${assetId}" asset='${assetJson}' display='${displayJson}'}\n\n:::\n`;
  },

  markdownTokenizer: {
    name: 'mediaView',
    level: 'block',
    start: (src) => src.indexOf(':::media'),
    tokenize: (src, tokens, lexer) => {
      const match = /^:::media \{assetId="([^"]*)" asset='([^']*)' display='([^']*)'\}\s*\n\s*\n:::\n?/.exec(src);
      if (!match) return undefined;

      const [raw, assetId, assetStr, displayStr] = match;
